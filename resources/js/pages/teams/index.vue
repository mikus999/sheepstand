      ...
      <v-card width="100%" :flat="$vuetify.breakpoint.xs">
        ...
        >
          ...
          <v-tab href="#tab-locations">
            ...
            <v-icon>{{ icons.mdiMapMarkerMultiple }}</v-icon>
          </v-tab>
                ...
                <v-col cols=12 sm=6 class="px-6">
                  ...
                  <v-text-field name="code" :label="$t('teams.team_code')" :value="teamData.code" readonly>
                    <template v-slot:append>
                      <v-icon @click="resetCode" color="error">{{ icons.mdiLockReset }}</v-icon>
                    </template>
                  </v-text-field>
                      ...
                      <div class="mt-8 mb-8" v-for="num in settings.shifts.numbers" :key="num.index">
                        <v-input class="ml-5">
                          <v-chip color="primary" class="mr-3">
                            <v-icon @click="teamData[num.column]-=num.step; changeSetting(num.column, 'num')" small left>{{ icons.mdiMinus }}</v-icon>
                            <span class="pa-1">{{ formatHoursMinutes(teamData[num.column]) }}</span>
                            <v-icon @click="teamData[num.column]+=num.step; changeSetting(num.column, 'num')" small right>{{ icons.mdiPlus }}</v-icon>
                          </v-chip>
